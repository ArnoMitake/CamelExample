...
public class ExportSMCDRDao extends JdbcTemplate {
	...
	public ExportSMCDRDao(DataSource dataSource) {
		super(dataSource);
	}

	public Object exportChtSMCDRToFile(String fileName) {
	...
	public Object exportFetSMCDRToFile(String fileName) {
        StringBuilder sql = new StringBuilder(100);
	...
	public Object exportTWMSMCDRToFile(String fileName) {
		StringBuilder sql = new StringBuilder(100);
	...
	public Object exportTWSMSSMCDRToFile(String fileName) {
		StringBuilder sql = new StringBuilder(100);

		sql.append(" SELECT dstaddr FROM dbo.SMCDRTWSMS with(nolock) ");
		sql.append(" WHERE donetime >= ? AND donetime < ? ");
		sql.append(" AND ( (len(dstaddr)=10 AND left(dstaddr,2)='09') ");
		sql.append(" OR (len(dstaddr)=12 AND left(dstaddr,4)='8869') ) ");
		sql.append(" AND statuscode='0' ");
	...
	public Object exportCCENTSMCDRToFile(String fileName) {
		StringBuilder sql = new StringBuilder(100);

		sql.append(" SELECT dstaddr FROM dbo.SMCDRCCENT with(nolock) ");
		sql.append(" WHERE donetime >= ? AND donetime < ? ");
		sql.append(" AND ( (len(dstaddr)=10 AND left(dstaddr,2)='09') ");
		sql.append(" OR (len(dstaddr)=12 AND left(dstaddr,4)='8869') ) ");
		sql.append(" AND statuscode='0' ");
